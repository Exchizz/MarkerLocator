...
class ROS:
	...
	self.toFind = rospy.get_param( '~toFind', '4')
        self.pub = None
#        self.markers = self.toFind
        self.bridge = CvBridge()

        self.pub = rospy.Publisher('positionPuplisher' + str(self.toFind), Point, queue_size = 10)

        self.imagePub = rospy.Publisher("imagePublisher", Image, queue_size=10)

	self.image_sub = rospy.Subscriber("/camera/stream",Image,self.callbackFrame)
	...
	self.cd = CameraDriver(self.toFind, defaultKernelSize = 15)
...
#        self.cd.drawDetectedMarkers()
	...
	print("x: %8.3f y: %8.3f angle: %8.3f quality: %8.3f order: %s" % (locations[0].x, locations[0].y, locations[0].theta, locations[0].quality, locations[0].order))
        self.pub.publish(  Point( locations[0].x, locations[0].y, locations[0].quality )  )
